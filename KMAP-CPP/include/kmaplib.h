#ifndef __kmaplib_h__
#define __kmaplib_h__
/*
 * kmaplib.h
 *
...
#include <stdio.h>
#include <math.h>
// #include <matrix.h> // TODO: matlab related
...
struct LevmarStats
{
  int n_loops   = 0;   // total while-loop passes
  int it_accept = 0;   // rho > 0
  int it_reject = 0;   // rho <= 0
  int hit_guard = 0;   // n > 1000 triggered
  int nan_ct    = 0;   // NaN observed in ct
  int nan_st    = 0;   // NaN observed in st
  double last_rho = 0.0;
  double last_mu  = 0.0;
};

// NaN test that works everywhere and is cheap
static inline bool kmap_isnan(double x) { return !(x == x); }
